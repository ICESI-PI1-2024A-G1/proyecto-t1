...
class RegistrationViewTest(TestCase):
    ...
    def test_register_view_post_existing_user(self):
        User.objects.create_user(username='123456', password='12345', email='test@u.icesi.edu.co')
        response = self.client.post(reverse('registration:register_view'), {'cedula': '123456', 'correo': 'test@u.icesi.edu.co', 'contrasena': '12345', 'confirmar_contrasena': '12345'})
    ...
    def test_register_view_post_invalid_cedula(self):
        response = self.client.post(reverse('registration:register_view'), {'cedula': '123', 'correo': 'test@u.icesi.edu.co', 'contrasena': '12345', 'confirmar_contrasena': '12345'})
        self.assertEqual(response.status_code, 200)  # Should render register.html with error message

    def test_register_view_post_invalid_email(self):
        response = self.client.post(reverse('registration:register_view'), {'cedula': '1234567', 'correo': 'test@gmail.com', 'contrasena': '12345', 'confirmar_contrasena': '12345'})
        self.assertEqual(response.status_code, 200)  # Should render register.html with error message

    def test_register_view_post_invalid_password(self):
        response = self.client.post(reverse('registration:register_view'), {'cedula': '1234567', 'correo': 'test@u.icesi.edu.co', 'contrasena': '123', 'confirmar_contrasena': '123'})
        self.assertEqual(response.status_code, 200)  # Should render register.html with error message

    def test_register_view_post_passwords_do_not_match(self):
        response = self.client.post(reverse('registration:register_view'), {'cedula': '1234567', 'correo': 'test@u.icesi.edu.co', 'contrasena': '12345678', 'confirmar_contrasena': '87654321'})
        self.assertEqual(response.status_code, 200)  # Should render register.html with error message

    def test_register_view_post_valid_user(self):
        response = self.client.post(reverse('registration:register_view'), {'cedula': '1234567', 'correo': 'test@u.icesi.edu.co', 'contrasena': '12345678', 'confirmar_contrasena': '12345678'})
        self.assertEqual(response.status_code, 302)  # Should redirect to verifyEmail_view

class VerifyEmailViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.client.session['random_code'] = 'ABC123'
