...
class LoginTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = get_user_model().objects.create_user(id='12345', password='12345', email='testuser@example.com', is_staff=True)

    def test_authenticate_valid_user(self):
        response = self.client.post(reverse('login:login_view'), {'usuario': '12345', 'contrasena': '12345'})
    ...
    def test_verify_email_valid_code(self):
        self.client.post(reverse('login:login_view'), {'usuario': '12345', 'contrasena': '12345'})
    ...
    def test_access_show_requests_without_login(self):
        response = self.client.get(reverse(requests_"/requests"))
